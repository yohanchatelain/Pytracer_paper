...
SciPy, and scikit-learn. \add{Only instrumented the Python interpreter would
    ...
    repeated the experiment with two MCA modes, Random Rounding (RR) and Full MCA,
    resulting in 2 experiments for each example}. We used the default virtual
...
\add{We can see on Figure}~\ref{fig:BRR_svd} \add{that the last singular values is
    ...
    following condition is never met \texttt{if iter\_ != 0 and
        np.sum(np.abs(coef\_old\_ - coef\_)) < self.tol:} since the comparison with a
