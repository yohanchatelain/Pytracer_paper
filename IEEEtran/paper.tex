...
\add{To detect numerical instabilities, \pytracer works in three main steps:}
\begin{enumerate}
\item It instruments Python programs and their module dependencies automatically, to save the input and output values of functions in a trace file.
\item It executes the instrumented program several times using Monte-Carlo Arithmetic to inject perturbations in function outputs.
\item It aggregates all trace files to compute statistics about the traced values, and visualize them in a GUI.
\end{enumerate}
\add{
    To implement these steps, \pytracer adopts the workflow shown in Figure~\ref{fig:workflow}.
At step 1, \pytracer uses \textit{monkey patching} to replace the functions called
by the application with a wrapper that saves their arguments and returned values
in a trace file. At step 2, it executes the application multiple times with Monte-Carlo Arithmetic as implemented in Fuzzy, a collection 
of scientific computing libraries recompiled with the Verificarlo tool. It then
aggregates the resulting trace files to compute summary statistics on
traced variable. At step 3, a Plotly dashboard provides interactive visualizations
