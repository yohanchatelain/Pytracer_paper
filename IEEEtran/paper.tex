...
\IEEEtitleabstractindextext{%
    \begin{abstract}
        ...
        noise model, allowing for \add{numerical profiling} through Monte-Carlo
...
stochastic  arithmetic,  uncertainty  or  sensitivity analysis \TG{[23] fix ref}, and random
seed analysis \TG{fix ref [11]}.  \add{PyTracer is agnostic to the noise model used and can therefore
...
numerical perturbations, whereas parsing errors occur when traces result from different 
control flow paths.} 
...
to denote the statistical variability measured in number of significant bits, in 
...
instability of the \texttt{gesdd} method mentioned in several GitHub issues \TG{(such as ...)},
including the LAPACK repository. Furthermore, it is worth mentioning that a Pull
Request has recently been submitted \TG{in the ... repo} to let the user select the SVD method and
