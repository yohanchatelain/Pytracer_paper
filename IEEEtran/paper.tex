...
\add{
    This limitation is expected to have a minor impact since non-writable
    classes are usually called from pure Python classes that can be
    instrumented. PyTracer reports information about the stability of
...
\begin{table*}[]
    ...
    \caption{\pytracer execution summary on SciPy and Scikit-learn examples.
        ...
        \pytracer trace parsing and aggregation, \emph{result}: \add{result} precision
         in number of significant bits. \valid: successful run, \remove{\warn: error
            from invalid noise injection,} \cross: numerical error \add{(tracing)} or 
            \add{different control flow paths (parsing).} \textit{nan} stands for (\texttt{Not-A-Number}). Example names
...
Table~\ref{tab:pytracer_results_summary} summarizes \pytracer's outcome on
SciPy and scikit-learn examples. Columns \textit{trace} and \textit{parse}
report on the tracing and aggregation steps with two possible outcomes:
\add{green \valid\xspace for success and red \cross\xspace for error. Tracing errors 
occur in the application as a result of 
numerical perturbations, whereas parsing errors occur in \pytracer when traces result from different 
control flow paths. 
} \remove{, and 3)
...
produces many outputs, the average value among them. \add{Significant bits are computed on the final program output 
and can therefore be computed even when parsing fails.} \add{We use the term ``result precision"
to denote a statistical variability measured in number of significant bits, in 
contrast with the other meanings of the word precision used in expressions
...
Figures~\ref{fig:fft1D_inputs} and~\ref{fig:fft1D_outputs} \add{show the
    ...
    \add{result} precision of real (blue points) and imaginary (orange points) parts.}
\add{The} top row in Figures~\ref{fig:fft1D_inputs} and~\ref{fig:fft1D_outputs}
show the input mean value while the bottom row shows the standard deviation. \add{Even though we 
used Random Rounding (RR), function inputs are perturbed since they result from multiple intermediate 
computations perturbed with RR.} The
...
\pytracer allows to quickly visualize the new \add{result} precision.
...
techniques used in code coverage analysis. \add{This would also enable a larger support 
of multi-threaded applications.} Moreover, we would like to
extend \pytracer to instrument elementary arithmetic operations \add{(+,-,*,/), to allow for more detailed analyses}.
\remove{Finally, although \pytracer supports multiple noise models, we only
tested it with Monte-Carlo arithmetic. Therefore, we would like to compare
different models against MCA.} \pytracer is available at
