...
\add{Hence, an application that consistently fails at the same instruction will
    produce traces with equivalent CFPs which can be parsed by \pytracer as the 
    Face Recognition shows.}
...
\pytracer without fuzzy \add{that corresponds to PyTracer used
    only for instrumenting Python programs to catch values of function inputs
    and outputs, but without instrumentation with MCA arithmetic.}
It is measured as the ratio between the actual execution time
...
sizes produced by \pytracer. \add{The p-value $\leq 10^{-8}$ shows a significant
    correlation between the log of the size and the log of the time.}
